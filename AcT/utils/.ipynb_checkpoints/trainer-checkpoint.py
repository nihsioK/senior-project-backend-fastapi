...
from utils.data import load_kinetics, load_mpose, one_hot, random_flip, random_noise
from utils.tools import CosineSchedule, CustomSchedule, Logger

# CUSTOM LIBRARIES
from utils.transformer import PatchClassEmbedding, Patches, TransformerEncoder
...
class Trainer:
    ...
    def build_act(self, transformer):
        inputs = tf.keras.layers.Input(
            shape=(
                self.config[self.config["DATASET"]]["FRAMES"]
                // self.config["SUBSAMPLE"],
                self.config[self.config["DATASET"]]["KEYPOINTS"]
                * self.config["CHANNELS"],
            )
        )
        x = tf.keras.layers.Dense(self.d_model)(inputs)
        x = PatchClassEmbedding(
            self.d_model,
            self.config[self.config["DATASET"]]["FRAMES"] // self.config["SUBSAMPLE"],
            pos_emb=None,
        )(x)
        x = transformer(x, False)
        x = tf.keras.layers.Lambda(lambda x: x[:, 0, :])(x)
        x = tf.keras.layers.Dense(self.mlp_head_size)(x)
        outputs = tf.keras.layers.Dense(self.config[self.config["DATASET"]]["CLASSES"])(
            x
        )
    ...
    def get_model(self):
        ...
        if self.config["SCHEDULER"]:
            lr = CustomSchedule(
                self.d_model,
                warmup_steps=self.train_steps
                * self.config["N_EPOCHS"]
                * self.config["WARMUP_PERC"],
                decay_step=self.train_steps
                * self.config["N_EPOCHS"]
                * self.config["STEP_PERC"],
            )
        ...
        optimizer = tf.keras.optimizers.AdamW(
            learning_rate=lr, weight_decay=self.config["WEIGHT_DECAY"]
        )
        ...
        self.checkpointer = tf.keras.callbacks.ModelCheckpoint(
            self.bin_path + self.name_model_bin,
    ...
    def get_data(self):
        if self.config["DATASET"] == "kinetics":
            train_gen, val_gen, test_gen, self.train_len, self.test_len = load_kinetics(
                self.config, self.fold
            )

            self.ds_train = tf.data.Dataset.from_generator(
                train_gen, output_types=("float32", "uint8")
            )
            self.ds_val = tf.data.Dataset.from_generator(
                val_gen, output_types=("float32", "uint8")
            )
            self.ds_test = tf.data.Dataset.from_generator(
                test_gen, output_types=("float32", "uint8")
            )

        else:
            X_train, y_train, X_test, y_test = load_mpose(
                self.config["DATASET"],
                self.split,
            ...
            print(f'Pose Extractor: {self.config["DATASET"]}')
        ...
        self.ds_train = self.ds_train.map(
            lambda x, y: one_hot(x, y, self.config[self.config["DATASET"]]["CLASSES"]),
        ...
        self.ds_train = self.ds_train.map(
            random_noise, num_parallel_calls=tf.data.experimental.AUTOTUNE
        )
    ...
    def get_random_hp(self):
        ...
        self.config["WEIGHT_DECAY"] = self.trial.suggest_float(
            "WD", 1e-5, 1e-3, step=1e-5
        )
        self.config["N_EPOCHS"] = int(
            self.trial.suggest_float("EPOCHS", 50, 100, step=10)
        )
        self.config["WARMUP_PERC"] = self.trial.suggest_float(
            "WARMUP_PERC", 0.1, 0.4, step=0.1
        )
        self.config["LR_MULT"] = self.trial.suggest_float("LR_MULT", -5, -4, step=1)
        self.config["SCHEDULER"] = self.trial.suggest_categorical(
    ...
    def do_training(self):
        ...
        accuracy_test, balanced_accuracy = self.evaluate(
            weights=self.bin_path + self.name_model_bin
        )
    ...
    def evaluate(self, weights=None):
        ...
        _, accuracy_test = self.model.evaluate(self.ds_test, steps=int(self.test_steps))
        if self.config["DATASET"] == "kinetics":
        ...
        y_pred = np.argmax(
            tf.nn.softmax(self.model.predict(tf.concat(X, axis=0)), axis=-1), axis=1
        )
        balanced_accuracy = sklearn.metrics.balanced_accuracy_score(
            tf.math.argmax(tf.concat(y, axis=0), axis=1), y_pred
        )
    ...
    def do_benchmark(self):
        for split in range(1, self.config["SPLITS"] + 1):
            ...
            np.save(
                ...
                + "_"
                + self.config["DATASET"]
                + f"_{split}_balanced_accuracy.npy",
                bal_acc_list,
            )
    ...
    def do_random_search(self):
        ...
        self.study = optuna.create_study(
            study_name="{}_random_search".format(self.config["MODEL_NAME"]),
            direction="maximize",
            pruner=pruner,
        )
        self.study.optimize(
            lambda trial: self.objective(trial), n_trials=self.config["N_TRIALS"]
        )
