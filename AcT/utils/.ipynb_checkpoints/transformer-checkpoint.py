...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask):
        ...
        batch_size = tf.cast(batch_size, dtype=tf.int32)  # Ensure it's an integer type
...
class PatchClassEmbedding(tf.keras.layers.Layer):
    def __init__(
        self, d_model, n_patches, pos_emb=None, kernel_initializer="he_normal", **kwargs
    ):
        ...
        if self.pos_emb is not None:
            ...
            self.lap_position_embedding = tf.keras.layers.Embedding(
                input_dim=self.pos_emb.shape[0], output_dim=self.d_model
            )
        else:
    ...
    def __call__(self, inputs):
        # Wrap the repeat in a Lambda layer so it can handle symbolic tensors.
        repeated_class_token = tf.keras.layers.Lambda(
            lambda inp: tf.repeat(self.class_embed, tf.shape(inp)[0], axis=0)
        )(inputs)
        x = tf.keras.layers.Lambda(lambda t: tf.concat(t, axis=1))([repeated_class_token, inputs])
